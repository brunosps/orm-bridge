...
export class BaseSql extends RawSQL implements AbstractSql {
  ...
  static async call<Q extends object>(
    ...
    const instance = new this(executor.getDatabaseType());

    const { query, params, queryCount } = instance.call(sqlParams);

    // Executa query de contagem
    const countResult = await executor.queryOne<{ TOTALROWS: number }>(
      queryCount,
      params,
    );
    ...
    const records = await executor.query<Q>(query, params);

    const meta = instance.calcMetadata(
      sqlParams.page ?? 1,
      sqlParams.perPage ?? instance.perPage(),
      totalRows,
    );
  ...
  calcMetadata(page: number, perPage: number, totalRows: number) {
    if (page === 0 || perPage === 0) {
      return {
        page: 0,
        perPage: 0,
  ...
  ): { whereClause: string; whereParams: { [x: string]: any } } {
